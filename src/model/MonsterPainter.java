...
public void draw(Graphics2D crayon_pac, int animationstage, int HEIGHT_INTERFACE) {
	...
	for(int i=0; i<monstres.length; i++) {
		try {
			...
			crayon_pac.drawImage(img, monstres[i].X, monstres[i].Y+HEIGHT_INTERFACE, monstres[i].X+Labyrinthe.Tile_length , monstres[i].Y+Labyrinthe.Tile_length+HEIGHT_INTERFACE, 0, 0, img.getWidth(null), img.getWidth(null), null);
	} catch (IOException e) {
