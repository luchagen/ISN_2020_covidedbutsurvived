...
public class LabyrinthePainter {
	Labyrinthe donjon;
	SpriteGroup staticsprites = new SpriteGroup("img/0/static.txt"); // on dessine avec un seul spritegroup toutes les cases du labyrinthe 
	...
	public LabyrinthePainter(Labyrinthe in_donjon, int WIDTH, int HEIGHT) {
		...
		//for(int i=0;i<nb_hauteur;i++) {
			//for(int j=0;j<nb_largeur;j++) {
				//if (donjon.isUnique(i, j))
					//dynsprites.add(new SpriteGroup(donjon.cases[i][j].skin)); //si la case testee doit avoir un sprite dynamique, on lui attribue un spritegroup dedie
		...
		for(int i=0;i<nb_largeur;i++) {
			for (int j=0;j<nb_hauteur;j++) {
				...
				try {
						...
						img = ImageIO.read(new File(staticsprites.currentSpriteGet(animationstage, donjon.cases[i][j].state, donjon.cases[i][j].type)));
